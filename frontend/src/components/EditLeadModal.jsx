...
const EditLeadModal = ({ leadId, onClose, onSuccess, employees }) => {
    ...
    const fetchLead = async () => {
      try {
        ...
        setFormData({
          ...
          lastContactedDate: data.lastContactedDate ? data.lastContactedDate.split('T')[0] : '',
          assignedEmployee: data.assignedEmployee?._id || data.assignedEmployee || ''
        });
      } catch (err) {
        setApiError('Failed to fetch lead data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleChange = (e) => {
    ...
    if (name === "stage") {
      setFormData({ ...formData, stage: value, subStage: '' });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/lead/${leadId}`, payload);
      onSuccess();
    } catch (err) {
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="modal-footer">
              ...
              <button type="submit" className="btn btn-primary" disabled={saving}>
                {saving ? <div className="spinner" style={{ width: '16px', height: '16px', borderWidth: '2px' }}></div> : 'Save Changes'}
              </button>
            </div>
          </form>
        )}
